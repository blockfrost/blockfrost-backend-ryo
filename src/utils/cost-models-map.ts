...
// https://github.com/IntersectMBO/plutus/blob/master/plutus-ledger-api/src/PlutusLedgerApi/V1/ParamName.hs#L17

const plutusV1Names: Array<string> = [
...
// https://github.com/IntersectMBO/plutus/blob/master/plutus-ledger-api/src/PlutusLedgerApi/V2/ParamName.hs#L17

const plutusV2Names: Array<string> = [
  'addInteger-cpu-arguments-intercept',
  'addInteger-cpu-arguments-slope',
  'addInteger-memory-arguments-intercept',
  'addInteger-memory-arguments-slope',
...
// https://github.com/IntersectMBO/plutus/blob/master/plutus-ledger-api/src/PlutusLedgerApi/V3/ParamName.hs#L17
...
export const costModelsMap = (costModels: any) => {
  ...
  if ('PlutusV1' in costModels && Object.keys(costModels.PlutusV1).length !== plutusV1Names.length)
    console.warn(
      'The size of the Plutus V1 cost model from db is',
      Object.keys(costModels.PlutusV1).length,
      'but our name mapping length is',
      plutusV1Names.length,
    );

  if ('PlutusV2' in costModels && Object.keys(costModels.PlutusV2).length !== plutusV2Names.length)
    console.warn(
      'The size of the Plutus V2 cost model from db is',
      Object.keys(costModels.PlutusV2).length,
      'but our name mapping length is',
      plutusV2Names.length,
    );

  if ('PlutusV3' in costModels && Object.keys(costModels.PlutusV3).length !== plutusV3Names.length)
    console.warn(
      'The size of the Plutus V3 cost model from db is',
      Object.keys(costModels.PlutusV3).length,
      'but our name mapping length is',
      plutusV3Names.length,
    );
  ...
  if ('PlutusV1' in costModels && validation.isNumber(Object.keys(costModels.PlutusV1)[0])) {
    ...
    for (let index = 0; index < plutusV1Names.length; index++)
      PlutusV1[plutusV1Names[index]] = costModels.PlutusV1[index];
  ...
  if ('PlutusV2' in costModels && validation.isNumber(Object.keys(costModels.PlutusV2)[0])) {
    ...
    for (let index = 0; index < plutusV2Names.length; index++)
      PlutusV2[plutusV2Names[index]] = costModels.PlutusV2[index];
