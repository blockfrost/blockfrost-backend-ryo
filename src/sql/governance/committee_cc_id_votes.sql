WITH credential AS (
  SELECT id
  FROM committee_hash
  WHERE raw = $4::bytea
    AND has_script = $5
),
target_hot_ids AS (
  SELECT id FROM credential WHERE $6 = 'hot'
  UNION
  SELECT DISTINCT cr.hot_key_id
  FROM committee_registration cr
  WHERE $6 = 'cold'
    AND cr.cold_key_id = (SELECT id FROM credential)
),
paged_votes AS (
  -- Pagination pushed before the joins; only fetch tx/block/proposal/anchor for the page.
  SELECT vp.id,
    vp.tx_id,
    vp.committee_voter,
    vp.gov_action_proposal_id,
    vp.voting_anchor_id,
    vp.vote
  FROM voting_procedure vp
  WHERE vp.committee_voter IN (SELECT id FROM target_hot_ids)
  ORDER BY CASE
      WHEN LOWER($1) = 'desc' THEN vp.id
    END DESC,
    CASE
      WHEN LOWER($1) <> 'desc'
      OR $1 IS NULL THEN vp.id
    END ASC
  LIMIT CASE
      WHEN $2 >= 1
      AND $2 <= 100 THEN $2
      ELSE 100
    END OFFSET CASE
      WHEN $3 > 1
      AND $3 < 2147483647 THEN ($3 - 1) * (
        CASE
          WHEN $2 >= 1
          AND $2 <= 100 THEN $2
          ELSE 100
        END
      )
      ELSE 0
    END
)
SELECT encode(tx.hash, 'hex') AS "tx_hash",
  encode(ch.raw, 'hex') AS "voter_hot_hex",
  ch.has_script AS "voter_hot_has_script",
  encode(ptx.hash, 'hex') AS "proposal_tx_hash",
  gap.index AS "proposal_index",
  (
    LOWER(
      regexp_replace(gap.type::TEXT, '(?<=.{1})([A-Z])', '_\1', 'g')
    )
  ) AS "governance_type",
  LOWER(p.vote::TEXT) AS "vote",
  va.url AS "metadata_url",
  encode(va.data_hash, 'hex') AS "metadata_hash",
  b.block_no AS "block_height",
  EXTRACT(EPOCH FROM b.time)::INTEGER AS "block_time"
FROM paged_votes p
  JOIN committee_hash ch ON (ch.id = p.committee_voter)
  JOIN tx ON (tx.id = p.tx_id)
  JOIN block b ON (b.id = tx.block_id)
  JOIN gov_action_proposal gap ON (gap.id = p.gov_action_proposal_id)
  JOIN tx AS ptx ON (ptx.id = gap.tx_id)
  LEFT JOIN voting_anchor va ON (va.id = p.voting_anchor_id)
ORDER BY CASE
    WHEN LOWER($1) = 'desc' THEN p.id
  END DESC,
  CASE
    WHEN LOWER($1) <> 'desc'
    OR $1 IS NULL THEN p.id
  END ASC
