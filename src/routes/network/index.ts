  ...
  fastify.route({
    ...
    handler: async (request: FastifyRequest, reply) => {
      ...
      if (genesisData && byronGenesisData) {
        ...
        try {
          const lastEpoch = await clientDbSync.query<QueryTypes.Epoch>(
            SQLQuery.get('network_epoch'),
          );
          ...
          const standardSafeZone = (securityParameter: number, activeSlotsCoeff: number) =>
            ...
            if (PROTOCOL_VERSIONS[epochProto.protocol_major].is_era_hardfork) {
              ...
              const next = {
                start: {
                  ...
                  time:
                    previous.end.time +
                    duration * genesisData.epoch_length * genesisData.slot_length,
          ...
          const last = {
            start: {
              ...
              time:
                previous.end.time +
                lastDuration * genesisData.epoch_length * genesisData.slot_length,
